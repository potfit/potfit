...
{
  if (ntypes <= 0)
    error("Missing parameter or invalid value in %s : ntypes is \"%d\"", paramfile,
      ntypes);

  if (strcmp(startpot, "\0") == 0)
    error("Missing parameter or invalid value in %s : startpot is \"%s\"", paramfile,
      startpot);
  ...
  if (strcmp(config, "\0") == 0)
    error("Missing parameter or invalid value in %s : config is \"%s\"", paramfile,
      config);

  if (strcmp(tempfile, "\0") == 0)
    error("Missing parameter or invalid value in %s : tempfile is \"%s\"", paramfile,
      tempfile);

  if (eweight < 0)
    error("Missing parameter or invalid value in %s : eweight is \"%f\"", paramfile,
      eweight);

#ifdef STRESS
  if (sweight < 0)
    error("Missing parameter or invalid value in %s : sweight is \"%f\"", paramfile,
      sweight);
#endif /* STRESS */
...
#ifdef APOT
  if (plotmin < 0)
    error("Missing parameter or invalid value in %s : plotmin is \"%f\"", paramfile,
      plotmin);
#ifdef PAIR
  ...
  if (strcmp(distfile, "\0") == 0)
    error("Missing parameter or invalid value in %s : distfile is \"%s\"", paramfile,
      distfile);
#endif /* PDIST */
}
