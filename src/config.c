...
/* check if there are enough atoms, 2 for pair and 3 for manybody potentials */
    ...
    do
    {
      ...
      if (res == NULL || feof(config_file))
        error(1, "Incomplete header in configuration file %s", filename);
        ...
        case 'B':
        {
          switch (res[3])
          {
            ...
            case 'A':
              ...
              cstate.have_contrib_box_vector &= 2;
              break;
            case 'b':
            case 'B':
              read_box_vector(res + 5, &cstate.cbox_b, &cstate);
              cstate.have_contrib_box_vector &= 4;
              break;
            case 'c':
            case 'C':
              read_box_vector(res + 5, &cstate.cbox_c, &cstate);
              cstate.have_contrib_box_vector &= 8;
              break;
            case 's':
            case 'S':
    ...
    // Addition by AI 17/7/2015
       ...
       g_config.lattice[g_config.nconf].xx = cstate.box_x.x;
       g_config.lattice[g_config.nconf].xy = cstate.box_x.y;
       g_config.lattice[g_config.nconf].xz = cstate.box_x.z;
       g_config.lattice[g_config.nconf].yx = cstate.box_y.x;
       g_config.lattice[g_config.nconf].yy = cstate.box_y.y;
       g_config.lattice[g_config.nconf].yz = cstate.box_y.z;
       g_config.lattice[g_config.nconf].zx = cstate.box_z.x;
       g_config.lattice[g_config.nconf].zy = cstate.box_z.y;
       g_config.lattice[g_config.nconf].zz = cstate.box_z.z;
    #endif
    // End addition
...
void create_memory_for_config(config_state* cstate)
{
  ...
  g_config.conf_weight = (double*)Realloc(g_config.conf_weight, nconf * sizeof(double));
...
#if defined(STRESS)
  g_config.stress = (sym_tens*)Realloc(g_config.stress, nconf * sizeof(sym_tens));
  ...
  g_config.na_type = (int**)Realloc(g_config.na_type, (nconf + 1) * sizeof(int*));
...
void read_box_vector(char const* pline, vector* pvect, config_state* cstate)
{
  if (sscanf(pline, "%lf %lf %lf\n", &pvect->x, &pvect->y, &pvect->z) == 3)
  {
    ...
    error(1, "%s: Error in box vector on line %d\n", cstate->filename, cstate->line);
}
...
void init_box_vectors(config_state* cstate)
{
  ...
  fprintf(stderr, "     %10.6f %10.6f %10.6f\n", cstate->tbox_x.x, cstate->tbox_x.y,
          cstate->tbox_x.z);
  fprintf(stderr, "     %10.6f %10.6f %10.6f\n", cstate->tbox_y.x, cstate->tbox_y.y,
          cstate->tbox_y.z);
  fprintf(stderr, "     %10.6f %10.6f %10.6f\n", cstate->tbox_z.x, cstate->tbox_z.y,
          cstate->tbox_z.z);
  fprintf(stderr, "Box heights:\n");
  fprintf(stderr, "     %10.6f %10.6f %10.6f\n", 1.0 / iheight.x, 1.0 / iheight.y,
          1.0 / iheight.z);
  fprintf(stderr, "Potential range:  %f\n", g_config.rcutmax);
  fprintf(stderr, "Periodic images needed: %d %d %d\n\n",
          (int)(2 * cstate->cell_scale.x + 1), (int)(2 * cstate->cell_scale.y + 1),
