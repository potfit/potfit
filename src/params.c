...
int read_parameter_file(char const *param_file)
{
  ...
  do {
    ...
    if (strcasecmp(token, "ntypes") == 0)
      return_error = get_param_int("ntypes", &g_param.ntypes, line, param_file,
                                   1, INT_MAX);
    ...
    /* prefix for general output files */
      ...
      rval = get_param_string("output_prefix", &g_files.output_prefix, line,
                              param_file);
    ...
    /* prefix for LAMMPS output files */
      ...
      rval = get_param_string("output_lammps", &g_files.output_lammps, line,
                              param_file);
    ...
    /* number of steps in IMD potential */
      ...
      return_error |= get_param_int("imdpotsteps", &g_param.imdpotsteps, line,
                                    param_file, 1, INT_MAX);

#if defined(APOT)
    /* minimum for plotfile */
    else if (strcasecmp(token, "plotmin") == 0)
      return_error |= get_param_double("plotmin", &g_param.plotmin, line,
                                       param_file, DBL_MIN, DBL_MAX);

#if defined(PAIR)
    /* exclude chemical potential from energy calculations */
    else if (strcasecmp(token, "enable_cp") == 0)
      return_error |= get_param_int("enable_cp", &g_param.enable_cp, line,
                                    param_file, INT_MIN, INT_MAX);
#endif  // PAIR
#endif  // APOT

    /* how far should the imd pot be extended */
    else if (strcasecmp(token, "extend") == 0)
      return_error |= get_param_double("extend", &g_param.extend, line,
                                       param_file, DBL_MIN, DBL_MAX);
    ...
    /* write radial pair distribution ? */
      ...
      return_error |= get_param_int("write_pair", &g_param.write_pair, line,
                                    param_file, 0, 1);

    /* plotpoint file */
    else if (strcasecmp(token, "plotpointfile") == 0)
      return_error |= get_param_string("plotpointfile", &g_files.plotpointfile,
                                       line, param_file);
    ...
    /* seed for RNG */
      ...
      return_error |= get_param_int("seed", &g_param.rng_seed, line, param_file,
                                    INT_MIN, INT_MAX);

#if defined(EVO)
    /* stopping criterion for differential evolution */
    else if (strcasecmp(token, "evo_threshold") == 0)
      return_error |= get_param_double("evo_threshold", &g_param.evo_threshold,
                                       line, param_file, 0, DBL_MAX);
#else  /* EVO */
    /* starting temperature for annealing */
    else if (strcasecmp(token, "anneal_temp") == 0)
      return_error |= get_param_string("anneal_temp", &g_param.anneal_temp,
                                       line, param_file);
#endif /* EVO */
    ...
    /* Energy Weight */
      ...
      return_error |= get_param_double("eng_weight", &g_param.eweight, line,
                                       param_file, 0, DBL_MAX);

    /* error margin delta epsilon */
    else if (strcasecmp(token, "d_eps") == 0)
      return_error |= get_param_double("d_eps", &g_calc.d_eps, line, param_file,
                                       0, DBL_MAX);
    ...
    /* write final potential in lammps format */
      ...
      return_error |= get_param_int("write_lammps", &g_param.write_lammps, line,
                                    param_file, 0, 1);

#if defined(COULOMB)
    /* cutoff-radius for long-range interactions */
    else if (strcasecmp(token, "dp_cut") == 0)
      return_error |= get_param_double("dp_cut", &g_todo.dp_cut, line,
                                       param_file, DBL_MIN, DBL_MAX);
#endif /* COULOMB */
    ...
    /* dipole iteration precision */
      ...
      return_error |= get_param_double("dp_tol", &g_todo.dp_tol, line,
                                       param_file, DBL_MIN, DBL_MAX);

    /* mixing parameter for damping dipole iteration loop */
    else if (strcasecmp(token, "dp_mix") == 0)
      return_error |= get_param_double("dp_mix", &g_todo.dp_mix, line,
                                       param_file, DBL_MIN, DBL_MAX);
#endif /* DIPOLE */

    /* global scaling parameter */
    else if (strcasecmp(token, "cell_scale") == 0)
      return_error |= get_param_double("cell_scale", &g_param.global_cell_scale,
                                       line, param_file, 0, DBL_MAX);
...
void check_parameters_complete(char const *paramfile)
{
  ...
  if (g_files.endpot == NULL) {
    warning("endpot is missing in %s, setting it to %s_end\n", paramfile,
            g_files.startpot);
    g_files.endpot =
        (char *)malloc((strlen(g_files.startpot) + 5) * sizeof(char));
