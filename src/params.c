...
int read_parameter_file(char const* param_file)
{
  ...
  do
  {
    ...
    /* number of steps in IMD potential */
      ...
      return_error |= get_param_int("imdpotsteps", &g_param.imdpotsteps, line, param_file,
                                    1, INT_MAX);

#if defined(APOT)
    /* minimum for plotfile */
    else if (strcasecmp(token, "plotmin") == 0)
      return_error |= get_param_double("plotmin", &g_param.plotmin, line, param_file,
                                       DBL_MIN, DBL_MAX);

#if defined(PAIR)
    /* exclude chemical potential from energy calculations */
    else if (strcasecmp(token, "enable_cp") == 0)
      return_error |= get_param_int("enable_cp", &g_param.enable_cp, line, param_file,
                                    INT_MIN, INT_MAX);
#endif  // PAIR
#endif  // APOT
    ...
    /* how far should the imd pot be extended */
      ...
      return_error |=
          get_param_double("extend", &g_param.extend, line, param_file, DBL_MIN, DBL_MAX);
    ...
    /* plotpoint file */
      ...
      return_error |=
          get_param_string("plotpointfile", &g_files.plotpointfile, line, param_file);
    ...
    /* seed for RNG */
      ...
      return_error |=
          get_param_int("seed", &g_param.rng_seed, line, param_file, INT_MIN, INT_MAX);

#if defined(EVO)
    /* stopping criterion for differential evolution */
    else if (strcasecmp(token, "evo_threshold") == 0)
      return_error |= get_param_double("evo_threshold", &g_param.evo_threshold, line,
    ...
    /* Scaling Constant for APOT Punishment */
      ...
      return_error |= get_param_double("apot_punish", &g_param.apot_punish_value, line,
                                       param_file, 0, DBL_MAX);
#endif /* APOT */
    ...
    /* Energy Weight */
      ...
      return_error |=
          get_param_double("eng_weight", &g_param.eweight, line, param_file, 0, DBL_MAX);
    ...
    /* Energy Weight */
      ...
      return_error |= get_param_double("stress_weight", &g_param.sweight, line,
                                       param_file, 0, DBL_MAX);
#endif /* STRESS */
    ...
    /* write final potential in lammps format */
      ...
      return_error |=
          get_param_int("write_lammps", &g_param.write_lammps, line, param_file, 0, 1);

#if defined(COULOMB)
    /* cutoff-radius for long-range interactions */
    else if (strcasecmp(token, "dp_cut") == 0)
      return_error |=
          get_param_double("dp_cut", &g_todo.dp_cut, line, param_file, DBL_MIN, DBL_MAX);
#endif /* COULOMB */
    ...
    /* dipole iteration precision */
      ...
      return_error |=
          get_param_double("dp_tol", &g_todo.dp_tol, line, param_file, DBL_MIN, DBL_MAX);

    /* mixing parameter for damping dipole iteration loop */
    else if (strcasecmp(token, "dp_mix") == 0)
      return_error |=
          get_param_double("dp_mix", &g_todo.dp_mix, line, param_file, DBL_MIN, DBL_MAX);
#endif /* DIPOLE */

    /* global scaling parameter */
    else if (strcasecmp(token, "cell_scale") == 0)
      return_error |= get_param_double("cell_scale", &g_param.global_cell_scale, line,
                                       param_file, 0, DBL_MAX);
...
{
  if (g_param.ntypes < 1)
    error(1, "Missing parameter or invalid value in %s : g_param.ntypes is \"%d\"",
  ...
  if (g_files.tempfile == NULL)
    error(1, "Missing parameter or invalid value in %s : tempfile is <undefined>",
          paramfile);

  if (g_param.eweight < 0)
    error(1, "Missing parameter or invalid value in %s : eng_weight is \"%d\"", paramfile,
          g_param.eweight);

#if defined(STRESS)
  if (g_param.sweight < 0)
    error(1, "Missing parameter or invalid value in %s : stress_weight is \"%d\"",
  ...
  if (g_param.plotmin < 0)
    error(1, "Missing parameter or invalid value in %s : plotmin is \"%d\"", paramfile,
          g_param.plotmin);
#if defined(PAIR)
  if (g_param.enable_cp != 0 && g_param.enable_cp != 1)
    error(1, "Missing parameter or invalid value in %s : enable_cp is \"%d\"", paramfile,
          g_param.enable_cp);
#endif /* PAIR */
#endif /* APOT */
  ...
  if (g_files.distfile == NULL)
    error(1, "Missing parameter or invalid value in %s : distfile is <undefined>",
          paramfile, g_files.distfile);
#endif /* PDIST */

  if (g_param.global_cell_scale <= 0)
    error(1, "Missing parameter or invalid value in %s : cell_scale is \"%d\"", paramfile,
          g_param.global_cell_scale);
}
