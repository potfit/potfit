...
void update_apot_table(double *xi)
{
  for (int i = 0; i < g_calc.ndim; i++)
    g_pot.apot_table.values[g_pot.apot_table.idxpot[i]][g_pot.apot_table.idxparam[i]] = xi[g_todo.idx[i]];
...
//   double f, h = 0;
  ...
  for (int i = 0; i < g_pot.calc_pot.ncols; i++)
  {
	...
	*(xi_calc + k) = g_pot.smooth_pot[i] ?
          f * cutoff(g_pot.calc_pot.xcoord[k], g_pot.apot_table.end[i], h) : f;
