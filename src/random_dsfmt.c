/****************************************************************
 *
 * random.c: Contains the dSFMT, a prng
 ...
 * @param w 128bit stracture of double precision floating point numbers (I/O)
 */
inline static void convert_o0c1(w128_t* w) { w->sd = _mm_sub_pd(sse2_double_two, w->sd); }
 ...
 */
static uint32_t ini_func1(uint32_t x) { return (x ^ (x >> 27)) * (uint32_t)1664525UL; }
 ...
 * @return 32-bit integer
 */
static uint32_t ini_func2(uint32_t x) { return (x ^ (x >> 27)) * (uint32_t)1566083941UL; }
