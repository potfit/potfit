  ...
  for (i = 0; i < g_param.ntypes; i++)
  {
    for (j = 0; j < g_param.ntypes; j++)
    {
      for (k = 0; k < g_param.ntypes; k++)
      {
        ...
        fprintf(outfile, " %g",
                g_pot.apot_table.values[g_calc.paircol + col_j][0] / sigma); /* gamma */
        ...
        fprintf(outfile, " %g", g_pot.apot_table.values[col_j][0] * scale_p /
                                    g_pot.apot_table.values[col_j][1] / scale_q); /* B */
        ...
        fprintf(outfile, " %g",
                *(g_pot.apot_table.tersoff.omega[col_j])); /* gamma_ijk = omega_ik */
        ...
        fprintf(outfile, " %g", 0.5 * (g_pot.apot_table.values[col_j][9] +
        ...
        fprintf(outfile, " %g", g_pot.apot_table.values[col_j][0]); /* A */
        fprintf(outfile, " %g",
                1.0 / (2.0 * g_pot.apot_table.values[col_j][5]));    /* n = 1/(2*delta) */
...
void write_pot_table_lammps()
{
  ...
  fprintf(outfile, "%d %f %d %f %f\n", g_param.imdpotsteps, drho, g_param.imdpotsteps, dr,
          g_config.rcutmin);
...
/* one block for every atom type */
  ...
  for (int i = 0; i < g_param.ntypes; i++)
  {
    ...
    for (int j = 0; j < g_param.imdpotsteps; j++)
    {
      ...
      temp =
          g_pot.smooth_pot[k]
              ? temp * cutoff(r, g_pot.apot_table.end[k],
