			 ...
			 reax_list **lists, output_controls *out_control )
{
 ...
 // printf("Coulomb energy calculation\n");
  ...
  for( i = 0; i < natoms; ++i ) {
	...
	{ // shielding
	  ...
	  e_vdW = (twbp->bom) * twbp->D * (exp1 - 2.0 * exp2);	  
	  ...
	  CEvd = dTap * e_vdW - 
	    Tap * (twbp->bom) * twbp->D * (twbp->alpha / twbp->r_vdW) * (exp1 - exp2) * dfn13;
	}
      else{ // no shielding
	...
	(twbp->bom) * C_ele * system->my_atoms[i].q * system->my_atoms[j].q * tmp;
		
      CEclmb = (twbp->bom) * C_ele * system->my_atoms[i].q * system->my_atoms[j].q * 
...
#endif
      /*printf("vdW: %6d%6d%12.4f%12.4f%12.4f\n",
	       system->my_atoms[i].orig_id, system->my_atoms[j].orig_id, 
	       r_ij, e_vdW, data->my_en.e_vdW );
      //fprintf(out_control->ecou,"%6d%6d%24.15e%24.15e%24.15e%24.15e%24.15e\n",
      printf("Coulomb: %6d%6d%12.4f%12.4f%12.4f%12.4f%12.4f\n",
	       system->my_atoms[i].orig_id, system->my_atoms[j].orig_id,
	       r_ij, system->my_atoms[i].q, system->my_atoms[j].q, 
	       e_ele, data->my_en.e_ele );*/
				   ...
				   output_controls *out_control )
{
  ...
  printf("Tabulted Coulomb energy calculation\n");
...
void Compute_Polarization_Energy( reax_system *system, simulation_data *data )
{
  ...
  for( i = 0; i < system->n; i++ ) {
    ...
    /* modification by ALBERT to switch off polarization */
    //def = system->reax_param.sbp[type_i].sigqeq;
  ...
  //printf("Polarization energy: %10.4f\n", data->my_en.e_pol);
}
