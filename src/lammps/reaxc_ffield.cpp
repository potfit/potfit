...
//	       reax->sbp[i].name );
  ...
  for (i=0; i < l; i++) {
    ...
    if (j < reax->num_atom_types && k < reax->num_atom_types) {
      ...
      val = atof(tmp[3]); 
      reax->tbp[k][j].bom = val; reax->tbp[j][k].bom = val;/* Albert modified for exclusion some Coulomb interactions */
      //printf("BOM:_%d_%d: %10.4f\n", k, j, reax->tbp[j][k].bom);                     
...
char Read_Force_Field2( double *param, reax_interaction *reax, 
		       control_params *control )
{
//  FILE    *fp;
//  char    *s;
//  char   **tmp;
  char ****tor_flag;
  int      c, i, j, k, l, m, n, o, p, cnt,offset;
  int num_bonds,num_off_diagonal,num_angles,num_torsion,num_hydrogen_bonds;
  int lgflag = control->lgflag;
  int errorflag = 1;
  real     val;
  MPI_Comm comm;
...
//  fgets( s, MAX_LINE, fp );
//  c = Tokenize( s, &tmp );
  reax->num_atom_types = (int)(param[40]);
...
//    c = Tokenize( s, &tmp );
    offset = 41 + i * 32;
//    for( j = 0; j < (int)(strlen(param[offset + 0])); ++j )
//      reax->sbp[i].name[j] = toupper( param[offset + 0][j] );
    strcpy(reax->sbp[i].name,"NULL");
    
    val = (param[offset + 0]); reax->sbp[i].r_s        = val;
    val = (param[offset + 1]); reax->sbp[i].valency    = val;
    val = (param[offset + 2]); reax->sbp[i].mass       = val;
    val = (param[offset + 3]); reax->sbp[i].r_vdw      = val;
    val = (param[offset + 4]); reax->sbp[i].epsilon    = val;
    val = (param[offset + 5]); reax->sbp[i].gamma      = val;
    val = (param[offset + 6]); reax->sbp[i].r_pi       = val;
    val = (param[offset + 7]); reax->sbp[i].valency_e  = val;
    reax->sbp[i].nlp_opt = 0.5 * (reax->sbp[i].valency_e-reax->sbp[i].valency);
...
//    c = Tokenize( s, &tmp );
    ...
    val = (param[offset + 8]); reax->sbp[i].alpha      = val;
    val = (param[offset + 9]); reax->sbp[i].gamma_w    = val;
    val = (param[offset + 10]); reax->sbp[i].valency_boc= val;
    val = (param[offset + 11]); reax->sbp[i].p_ovun5    = val;
//    val = (param[offset + 12]); 
    val = (param[offset + 13]); reax->sbp[i].chi        = val;
    val = (param[offset + 14]); reax->sbp[i].eta        = 2.0 * val;
    val = (param[offset + 15]); reax->sbp[i].p_hbond = (int) val;
    
    /* line 3 */
//    fgets( s, MAX_LINE, fp );
//    c = Tokenize( s, &tmp );
    
    val = (param[offset + 16]); reax->sbp[i].r_pi_pi    = val;
    val = (param[offset + 17]); reax->sbp[i].p_lp2      = val;
//    val = (param[offset + 18]); 
    val = (param[offset + 19]); reax->sbp[i].b_o_131    = val;
    val = (param[offset + 20]); reax->sbp[i].b_o_132    = val;
    val = (param[offset + 21]); reax->sbp[i].b_o_133    = val;
    val = (param[offset + 22]); reax->sbp[i].sigqeq     = val;
...
/*    if (c < 3) {
    ...
    val = (param[offset + 24]); reax->sbp[i].p_ovun2    = val;
    val = (param[offset + 25]); reax->sbp[i].p_val3     = val;
//    val = (param[offset + 26]); 
    val = (param[offset + 27]); reax->sbp[i].valency_val= val;
    val = (param[offset + 28]); reax->sbp[i].p_val5     = val;
    val = (param[offset + 29]); reax->sbp[i].rcore2     = val;
    val = (param[offset + 30]); reax->sbp[i].ecore2     = val;
    val = (param[offset + 31]); reax->sbp[i].acore2     = val;
...
//      val = (param[0]); reax->sbp[i].lgcij   	= val;
//      val = (param[1]); reax->sbp[i].lgre   	= val;
    }
...
//  fgets(s,MAX_LINE,fp);
//  c=Tokenize(s,&tmp);
  l = (int)(param[41 + 32 * reax->num_atom_types]);
  num_bonds = l;
  /* a line of comments */
//  fgets(s,MAX_LINE,fp);
  
  for (i=0; i < l; i++) {
  offset = 42 + 32 * reax->num_atom_types + 18 * i;
    /* line 1 */
//    fgets(s,MAX_LINE,fp);
//    c=Tokenize(s,&tmp);
      
    j = (int)(param[offset + 0]) - 1;
    k = (int)(param[offset + 1]) - 1;

    if (j < reax->num_atom_types && k < reax->num_atom_types) {

      val = (param[offset + 2]); reax->tbp[j][k].De_s      = val;
      reax->tbp[k][j].De_s      = val;
      val = (param[offset + 3]); reax->tbp[j][k].De_p      = val;
      reax->tbp[k][j].De_p      = val;
      val = (param[offset + 4]); reax->tbp[j][k].De_pp     = val;
      reax->tbp[k][j].De_pp     = val;
      val = (param[offset + 5]); reax->tbp[j][k].p_be1     = val;
      reax->tbp[k][j].p_be1     = val;
      val = (param[offset + 6]); reax->tbp[j][k].p_bo5     = val;
      reax->tbp[k][j].p_bo5     = val;
      val = (param[offset + 7]); reax->tbp[j][k].v13cor    = val;
      reax->tbp[k][j].v13cor    = val;
                          
      val = (param[offset + 8]); reax->tbp[j][k].p_bo6     = val;
      reax->tbp[k][j].p_bo6     = val;
      val = (param[offset + 9]); reax->tbp[j][k].p_ovun1 = val;
      reax->tbp[k][j].p_ovun1 = val;
...
//      c=Tokenize(s,&tmp);
      ...
      val = (param[offset + 10]); reax->tbp[j][k].p_be2     = val;
      reax->tbp[k][j].p_be2     = val;
      val = (param[offset + 11]); reax->tbp[j][k].p_bo3     = val;
      reax->tbp[k][j].p_bo3     = val;
      val = (param[offset + 12]); reax->tbp[j][k].p_bo4     = val;
      reax->tbp[k][j].p_bo4     = val;
//      val = (param[offset + 13]); 
                         
      val = (param[offset + 13]); reax->tbp[k][j].bom = val; 
      reax->tbp[j][k].bom = val;/* Albert modified for exclusion some Coulomb interactions */
      //printf("BOM:_%d_%d: %10.4f\n", k, j, reax->tbp[j][k].bom);

 
      val = (param[offset + 14]); reax->tbp[j][k].p_bo1     = val;
      reax->tbp[k][j].p_bo1     = val;
      val = (param[offset + 15]); reax->tbp[j][k].p_bo2     = val;
      reax->tbp[k][j].p_bo2     = val;
      val = (param[offset + 16]); reax->tbp[j][k].ovc       = val;
      reax->tbp[k][j].ovc       = val;
...
//  fgets(s,MAX_LINE,fp);
//  c=Tokenize(s,&tmp);
  l = (int)(param[42 + 32 * reax->num_atom_types + 18 * num_bonds]);
  num_off_diagonal = l;
  for (i=0; i < l; i++) {
  offset = 43 + 32 * reax->num_atom_types + 18 * num_bonds + 8 * i;
//    fgets(s,MAX_LINE,fp);
//    c=Tokenize(s,&tmp);
...
//  fgets( s, MAX_LINE, fp );
//  c = Tokenize( s, &tmp );
  l = (int)( param[43 + 32 * reax->num_atom_types + 18 * num_bonds + 8 * num_off_diagonal] );
  num_angles = l;
  
  for( i = 0; i < l; i++ ) {
  offset = 44 + 32 * reax->num_atom_types + 18 * num_bonds + 8 * num_off_diagonal + 10 * i;
//    fgets(s,MAX_LINE,fp);
//    c=Tokenize(s,&tmp);
      
    j = (int)(param[offset + 0]) - 1;
    k = (int)(param[offset + 1]) - 1;
    m = (int)(param[offset + 2]) - 1;
...
//  fgets( s, MAX_LINE, fp );
//  c = Tokenize( s, &tmp );
  l = (int)( param[44 + 32 * reax->num_atom_types + 18 * num_bonds + 8 * num_off_diagonal + 10 * num_angles] );
  num_torsion = l;

  for( i = 0; i < l; i++ ) {
  offset = 45 + 32 * reax->num_atom_types + 18 * num_bonds + 8 * num_off_diagonal + 10 * num_angles + 11 * i;
//    fgets( s, MAX_LINE, fp );
//    c = Tokenize( s, &tmp );
	...
	for( p = 0; p < reax->num_atom_types; p++ )
	  for( o = 0; o < reax->num_atom_types; o++ ) {
	    ...
	    if (tor_flag[p][k][m][o] == 0) {
	      reax->fbp[p][k][m][o].prm[0].V1 = (param[offset + 4]);
	      reax->fbp[p][k][m][o].prm[0].V2 = (param[offset + 5]);
	      reax->fbp[p][k][m][o].prm[0].V3 = (param[offset + 6]);
	      reax->fbp[p][k][m][o].prm[0].p_tor1 = (param[offset + 7]);
	      reax->fbp[p][k][m][o].prm[0].p_cot1 = (param[offset + 8]);
	    }
		  
	    if (tor_flag[o][m][k][p] == 0) {
	      reax->fbp[o][m][k][p].prm[0].V1 = (param[offset + 4]);
	      reax->fbp[o][m][k][p].prm[0].V2 = (param[offset + 5]);
	      reax->fbp[o][m][k][p].prm[0].V3 = (param[offset + 6]);
	      reax->fbp[o][m][k][p].prm[0].p_tor1 = (param[offset + 7]);
	      reax->fbp[o][m][k][p].prm[0].p_cot1 = (param[offset + 8]);
	    }		
	  }
    }
  }
...
//  fgets( s, MAX_LINE, fp );
//  c = Tokenize( s, &tmp );
  l = (int)( param[45 + 32 * reax->num_atom_types + 18 * num_bonds + 8 * num_off_diagonal + 10 * num_angles + 11 * num_torsion] );
  num_hydrogen_bonds = l;
  
  for( i = 0; i < l; i++ ) {
   offset = 46 + 32 * reax->num_atom_types + 18 * num_bonds + 8 * num_off_diagonal + 10 * num_angles + 11 * num_torsion + 7 * i; 
//    fgets( s, MAX_LINE, fp );
//    c = Tokenize( s, &tmp );
