...
/* Not much sense in printing rho when not communicated... */
  ...
  for (i = 0; i < 3 * g_config.natoms; i++) {
    ...
    f_sum += sqr;
#ifdef FWEIGHT
    if (i > 2 && i % 3 == 0 && g_config.atoms[i / 3].conf != g_config.atoms[i / 3 - 1].conf)
      fprintf(outfile, "\n\n");
    if (i == 0)
  ...
  if (g_param.eweight != 0) {
    ...
    for (i = 0; i < g_config.nconf; i++) {
      sqr = g_config.conf_weight[i] * g_param.eweight * dsquare(force[g_calc.energy_p + i]);
      e_sum += sqr;
      if (g_param.write_output_files) {
	fprintf(outfile, "%3d\t%6.2f\t%10.6f\t%13.10f\t%13.10f\t%f\t%f\t%f\n",
                i, g_config.conf_weight[i], sqr / g_config.conf_weight[i],
         force[g_calc.energy_p + i] + g_config.force_0[g_calc.energy_p + i], g_config.force_0[g_calc.energy_p + i],
           fabs(force[g_calc.energy_p + i]), force[g_calc.energy_p + i], force[g_calc.energy_p + i] / g_config.force_0[g_calc.energy_p + i]);
      } else
  ...
  if (g_param.sweight != 0) {
    ...
    for (i = g_calc.stress_p; i < g_calc.stress_p + 6 * g_config.nconf; i++) {
      sqr = g_config.conf_weight[(i - g_calc.stress_p) / 6] * g_param.sweight * dsquare(force[i]);
      s_sum += sqr;
      fprintf(outfile, "%3d-%s\t%6.2f\t%14.8f\t%12.10f\t%12.10f\t%14.8f\n",
	(i - g_calc.stress_p) / 6, component[(i - g_calc.stress_p) % 6],
	g_config.conf_weight[(i - g_calc.stress_p) / 6], sqr, force[i] + g_config.force_0[i], g_config.force_0[i], force[i] / g_config.force_0[i]);
    }
    if (g_param.write_output_files) {
  ...
  if (g_param.write_output_files) {
    fprintf(outfile, "sum of force-errors  = %e\t\t( %.3f%% - av: %f)\n",
            f_sum, f_sum / tot * 100, f_sum / (3 * g_config.natoms));
    if (g_param.eweight != 0)
