        ...
        for (int i = 0; i < g_config.inconf[h]; i++) {
          ...
          for (j = 0; j < atom->num_neigh; j++) {
            ...
            if (atom->type == neigh->type) {
              ...
              if (neigh->r <
                  g_pot.calc_pot
                      .end[g_calc.paircol + 2 * g_param.ntypes + atom->type]) {
                g_config.conf_atoms[neigh->nr - g_mpi.firstatom].rho_s +=
                    (*g_splint)(
                        &g_pot.calc_pot, xi,
                        g_calc.paircol + 2 * g_param.ntypes + atom->type,
                        neigh->r);
              }
#endif  // TBEAM
            }
          ...
          if (atom->rho_s > g_pot.calc_pot.end[col_F_s]) {
            ...
            forces[g_calc.limit_p + h] +=
                DUMMY_WEIGHT * 10.0 *
                dsquare(atom->rho_s - g_pot.calc_pot.end[col_F_s]);
...
/* contribution to cohesive energy is F(n) */
          ...
          if (atom->rho_s < g_pot.calc_pot.begin[col_F_s]) {
            ...
            atom->gradF_s = apot_gradient(atom->rho_s,
                                          xi_opt + g_pot.opt_pot.first[col_F_s],
                                          g_pot.apot_table.fvalue[col_F_s]);
            forces[g_calc.energy_p + h] += temp_eng;
#else
            /* linear extrapolation left */
            rho_s_val =
                (*g_splint_comb)(&g_pot.calc_pot, xi, col_F_s,
                                 g_pot.calc_pot.begin[col_F_s], &atom->gradF_s);
            forces[g_calc.energy_p + h] +=
                rho_s_val +
                (atom->rho_s - g_pot.calc_pot.begin[col_F_s]) * atom->gradF_s;
#endif  // APOT
            ...
            rho_s_val = (*g_splint_comb)(&g_pot.calc_pot, xi, col_F_s,
                                         g_pot.calc_pot.end[col_F_s] -
                                             0.5 * g_pot.calc_pot.step[col_F_s],
                                         &atom->gradF_s);
            forces[g_calc.energy_p + h] +=
                rho_s_val +
                (atom->rho_s - g_pot.calc_pot.end[col_F_s]) * atom->gradF_s;
#endif  // APOT
          }
          /* and in-between */
          else {
            ...
            if (atom->rho_s < 0.1) {
              g_pot.apot_table.fvalue[col_F_s](
                  atom->rho_s, xi_opt + g_pot.opt_pot.first[col_F_s],
                  &temp_eng);
              atom->gradF_s = apot_gradient(
                  atom->rho_s, xi_opt + g_pot.opt_pot.first[col_F_s],
                  g_pot.apot_table.fvalue[col_F_s]);
              forces[g_calc.energy_p + h] += temp_eng;
            } else
...
#if defined(TBEAM) /* s-band contribution to force for TBEAM */
                if ((r < g_pot.calc_pot.end[neigh->col[2]]) ||
                    (r < g_pot.calc_pot.end[col_F_s - g_param.ntypes])) {
                  rho_s_grad =
                      (r < g_pot.calc_pot.end[neigh->col[2]])
...
#if defined(TBEAM)
    rho_sum_temp = 0.0;
    MPI_Reduce(&rho_s_sum, &rho_sum_temp, 1, MPI_DOUBLE, MPI_SUM, 0,
               MPI_COMM_WORLD);
    if (g_mpi.myid == 0)
      rho_s_sum = rho_sum_temp;
#endif  // TBEAM
#endif  // MPI
...
/* dummy constraints (global) */
    ...
    if (g_mpi.myid == 0) {
      ...
      for (g = 0; g < g_param.ntypes; g++) {
        ...
        forces[g_calc.dummy_p + 2 * g_param.ntypes + g] =
            DUMMY_WEIGHT *
                (*g_splint_grad)(
                    &g_pot.calc_pot, xi,
                    g_calc.paircol + 3 * g_param.ntypes + g,
                    0.5 * (g_pot.calc_pot
                               .begin[g_calc.paircol + 3 * g_param.ntypes + g] +
                           g_pot.calc_pot
                               .end[g_calc.paircol + 3 * g_param.ntypes + g])) -
      ...
      if (rho_sum > 0.0) {
         ...
         */
        forces[g_calc.dummy_p + g_param.ntypes] =
            DUMMY_WEIGHT * (rho_sum - 1.0);
      ...
      if (rho_s_sum > 0.0) {
        ...
        /* ATTN: if there are invariant potentials, things might be problematic
         */
        forces[g_calc.dummy_p + 3 * g_param.ntypes] =
            DUMMY_WEIGHT * (rho_s_sum - 1.0);
