...
void init_population(double **pop, double *xi, double *cost)
{
  ...
  for (i = 0; i < NP; i++) cost[i] = (*g_calc_forces)(pop[i], fxi, 0);
#ifdef APOT
...
void opposite_check(double **P, double *costP, int init)
{
  ...
  if (!init) {
    ...
    for (i = 0; i < NP; i++) {
      for (j = 0; j < g_calc.ndim; j++) {
        if (P[i][g_todo.idx[j]] < minp[j]) minp[j] = P[i][g_todo.idx[j]];
        if (P[i][g_todo.idx[j]] > maxp[j]) maxp[j] = P[i][g_todo.idx[j]];
      }
    }
  }
...
/*            trial[D-2] * (x1[a][j] + x1[b][j] - x1[c][j] - x1[d][j]);*/
  ...
  for (j = 0; j < g_calc.ndimtot; j++) xi[j] = best[j];
