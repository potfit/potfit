...
void init_population(double **pop, double *xi, double *cost)
{
  ...
  for (i = 1; i < NP; i++) {
    for (j = 0; j < g_calc.ndim; j++) {
      val = xi[g_todo.idx[j]];
#ifdef APOT
      min = g_pot.apot_table.pmin[g_pot.apot_table.idxpot[j]][g_pot.apot_table.idxparam[j]];
      max = g_pot.apot_table.pmax[g_pot.apot_table.idxpot[j]][g_pot.apot_table.idxparam[j]];
...
/*          temp = x1[c][idx[j]] + trial[D - 2] * (x1[a][idx[j]] - x1[b][idx[j]]);*/
	  /* DE/best/1/exp */
          temp = best[g_todo.idx[j]] + trial[D - 2] * (x1[a][g_todo.idx[j]] - x1[b][g_todo.idx[j]]);
	  /* DE/rand/2/exp */
...
#ifdef APOT
      pmin = g_pot.apot_table.pmin[g_pot.apot_table.idxpot[j]][g_pot.apot_table.idxparam[j]];
      pmax = g_pot.apot_table.pmax[g_pot.apot_table.idxpot[j]][g_pot.apot_table.idxparam[j]];
	  if (temp > pmax) {
