...

{
  ...
  if (f_vec3 == NULL) f_vec3= vect_real(mdim);
  
  p_left=f_vec1; p_right=f_vec2; p_center=f_vec3;
  ...
  if (f_right >= f_left) 
    {
      ...
      nb_eval++;
      for (j=0;j<ndimtot;j++) vecu[j]=xicom[j]+x_left*delcom[j];/*set vecu */
  ...
  else
    {
      ...
      nb_eval++;
      for (j=0;j<ndimtot;j++) vecu[j]=xicom[j]+x_right*delcom[j];/*set vecu */
  ...
  do
      ...
      if (f_center < f_left )
        {
	      ...
	      /* SUCCESS: Minimum in bracket! */
              ...
              *f_upper = f_right;
              *f_minimum = f_center;
	      for (j=0;j<mdim;j++) f_vec1[j]=p_center[j];
              return ;
            }
	      ...
	      P_SWAP(p_left, p_center,p_temp);
              ...
              x_right = (x_center - x_left) / CGOLD + x_left;
              nb_eval++;
	      for (j=0;j<ndimtot;j++) vecu[j]=xicom[j]+x_right*delcom[j];
	      f_right=(*calc_forces)(vecu,p_right,0); 
            }
	      ...
	      x_center = -(x_right - x_left) * CGOLD + x_right;
              nb_eval++;
	      for (j=0;j<ndimtot;j++) vecu[j]=xicom[j]+x_center*delcom[j];
	...
	/* Search to the left */
	  ...
	  P_SWAP(p_center,p_left,p_temp);
          x_left = -(x_right - x_center) / CGOLD + x_right;
          nb_eval++;
	  for (j=0;j<ndimtot;j++) vecu[j]=xicom[j]+x_left*delcom[j];
	  f_left  =(*calc_forces)(vecu,p_left ,0); 
        }
