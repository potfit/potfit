...
atom_t *read_config(char *filename)
{
  ...
  while (end_header == 0) {
    ...
    /* box_x */
      ...
      if (3 != sscanf((const char *)(buffer + 2), "%lf %lf %lf", &box_x.x,
	  &box_x.y, &box_x.z)) {
	sprintf(msg, "Corrupt header in file %s", filename);
	error(msg);
      }
    ...
    /* box_y */
      ...
      if (3 != sscanf((const char *)(buffer + 2), "%lf %lf %lf", &box_y.x,
	  &box_y.y, &box_y.z)) {
	sprintf(msg, "Corrupt header in file %s", filename);
	error(msg);
      }
    ...
    /* box_z */
      ...
      if (3 != sscanf((const char *)(buffer + 2), "%lf %lf %lf", &box_z.x,
	  &box_z.y, &box_z.z)) {
	sprintf(msg, "Corrupt header in file %s", filename);
	error(msg);
      }
  ...
  if (atoms == NULL)
    error("Cannot allocate atoms list");

  /* allocate neighbor tables */
  for (i = 0; i < natoms; i++) {
    atoms[i].neigh = (neigh_t *)malloc(MAXNEIGH * sizeof(neigh_t));
    if (atoms[i].neigh == NULL)
      error("Cannot allocate neighbor list");
    atoms[i].n_neigh = 0;
  }
  ...
  for (i = 0; i < natoms; i++) {
    ...
    fscanf(infile, "%d %lf %lf %lf %lf %lf %lf\n", &atom->typ, &atom->pos.x,
      &atom->pos.y, &atom->pos.z, &atom->force.x, &atom->force.y,
      &atom->force.z);
  }
  fclose(infile);
  return atoms;
