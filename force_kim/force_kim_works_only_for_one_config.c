...
printf(" %15.5e  %15.5e  %15.5e  %15.5e \n", phi_grad, neigh->dist_r.x, neigh->dist_r.y, neigh->dist_r.z  );
*/
/* unittest ends */
...
*/
	...
	for (i = 0; i < DIM*inconf[h]; i++) {
		n_i = cnfstart[h] + i;
	  if (uf) {
	    forces[n_i] = kimforce[i] - force_0[n_i];
		} else {
		  forces[n_i] = 0.0;
	  }
		kim_tmpsum += conf_weight[h]* dsquare(forces[n_i]);
	}
...
int CreateKIMObj(void* pkim, int Natoms, int Nspecies, int start, char* modelname)
{
  ...
  int status;	
	...
	*/	
	int neighListLength; /* length of neighList */
	/* get the model name */
  ...
  return status;
	...
	for (i = 0; i < *numberOfParticles; i++) {
		...
		for (j = 0; j < atoms[start+i].num_neigh; j++) {
			NeighObject->neighborList[k]  =	atoms[start+i].neigh[j].nr;
