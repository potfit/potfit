/****************************************************************
*
* force.c: Routine used for calculating meam forces/energies
...
*	      MEAM-potential: Sergey Starikov, Vladimir Stegailov
...
/*****************************************************************************
*
*  compute forces using meam potentials with spline interpolation
*
...
#pragma omp parallel
       ...
       also OMP-parallelized region */
    {
      ...
      atom_t *atom;
      neigh_t *neigh;
      int   n_ne, n_j, n_k, ijk, typ_j, typ_k, col3, col4, col5;
      real  f_ij, f_ik, m_ijk, gradff, gradff2, gradmm, gradmm2, cos_ijk,
	dcos_ij, dcos_ik;
      real  angular_force_x, angular_force_y, angular_force_z, r_ij, r_ik,
	r_jk, dcos_ji, dcos_jk_x, dcos_jk_y, dcos_jk_z, cos, cos_jik, f_jk;
      real  tm_force_x, tm_force_y, tm_force_z;
      angl *n_angl;
//      FILE  *force_help;
#ifdef _OPENMP
...
/* then transfer(a->b)==transfer(b->a) */
	  ...
	  /* angular part of dens */
	  n_ne = atom->n_neigh;
	  n_ne = (n_ne - 1) * n_ne * 0.5;
	  for (ijk = 0; ijk < n_ne; ijk++) {
	    n_angl = atom->angl_part + ijk;
	    typ_j = n_angl->typ2;
	    typ_k = n_angl->typ3;
	    col3 = (typ1 <= typ_j) ?
	      paircol + 2 * ntypes + typ1 * ntypes + typ_j -
	      ((typ1 * (typ1 + 1)) / 2)
	      : paircol + 2 * ntypes + typ_j * ntypes + typ1 -
	      ((typ_j * (typ_j + 1)) / 2);
	    col4 =
	      (typ1 <=
...
/* then we use the final value, with PARABEL: extrapolate */
	...
	if (uf) {		/* only required if we calc forces */
	  for (i = 0; i < inconf[h]; i++) {
	    ...
	    tm_force_y = 0;
	    tm_force_z = 0;
	    for (ijk = 0; ijk < n_ne; ijk++) {	/* angular part */
	      n_angl = atom->angl_part + ijk;
	      typ_j = n_angl->typ2;
	      typ_k = n_angl->typ3;
	      r_ij = n_angl->r2;
	      ...
	      n_k = n_angl->nr3;
	      cos = n_angl->cos;
	      dcos_ij = n_angl->dcos_ij;
	      dcos_ik = n_angl->dcos_ik;
	      dcos_jk_x = n_angl->dcos_jk_x;
	      dcos_jk_y = n_angl->dcos_jk_y;
	      dcos_jk_z = n_angl->dcos_jk_z;
	      col3 = (typ1 <= typ_j) ?
		paircol + 2 * ntypes + typ1 * ntypes + typ_j -
		((typ1 * (typ1 + 1)) / 2)
		: paircol + 2 * ntypes + typ_j * ntypes + typ1 -
...
/* without angl part */
	    ...
	    forces[k] += angular_force_x;
	    forces[k + 1] += angular_force_y;
	    forces[k + 2] += angular_force_z;
	  }			/* third loop over atoms */
	  for (i = 0; i < inconf[h]; i++) {
	    atom = conf_atoms + i + cnfstart[h] - firstatom;
	    typ1 = atom->typ;
	    k = 3 * (cnfstart[h] + i);
//          force_help = fopen("/home/sstar/potfit/force.txt", "a");
//          fclose(force_help);
#ifdef FWEIGHT
